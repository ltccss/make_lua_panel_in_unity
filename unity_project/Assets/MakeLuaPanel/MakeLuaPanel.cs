...
public class MakeLuaPanel : MonoBehaviour 
{
    ...
    /// the target lua class name, the default value is {C#ClassName} + "Panel" if empty
    /// </summary>
    [Header("Default:{C#ClassName}+\"Panel\"")]
    public string luaClassName;
    /// <summary>
    /// default value is "_FindComponent"
    /// NOT recommended to change this value because probably you should modify the template lua file at the same time, 
    ...
    public void MakeLuaPanelFile()
    {
        ...
        string luaFunctionName = string.IsNullOrEmpty(this.luaFunctionName)? "_FindComponent" : this.luaFunctionName;
        ...
        if (fileText != null)
        {
            if (CheckIfHasLuaFunction(luaClassName, luaFunctionName, fileText))
            {
                int startIndex = -1;
                int length = 0;

                GetLuaFunctionTextPosition(luaClassName, luaFunctionName, fileText, out startIndex, out length);

                fileText = fileText.Substring(0, startIndex)
                    + GenerateFindComponentContext(memberList, this, luaClassName, luaFunctionName, selfAlias, false) 
                    + (((startIndex + length) == fileText.Length)? "" : fileText.Substring(startIndex + length, fileText.Length - (startIndex + length)));

            }
            else
            {
                fileText += "\n" + GenerateFindComponentContext(memberList, this, luaClassName, luaFunctionName, selfAlias);
            }
        }
    ...
    /// needWrap:是否包括函数头和尾
    /// </summary>
    private string GenerateFindComponentContext(List<MemberInfo> memberInfoList, MakeLuaPanel panel, string luaClassName, string luaFunctionName, string selfAlias, bool needWrap = true)
    {
        string result = "";

        if (needWrap)
        {
            result += string.Format("function {0}:{1}(pGameObject)", luaClassName, luaFunctionName);
            result += "\n";
        }
        ...
        for (int i = 0; i < memberInfoList.Count; i++)
        {
            ...
            {
                FieldInfo fd = memberInfoList[i] as FieldInfo;
                var comp = fd.GetValue(this) as Component;
            ...
            else if (fieldType == FieldType.List)
            {
                ...
                if (elementGenericTypeArray != null && elementGenericTypeArray.Length == 1)
                {
                    ...
                    var listObj = fd.GetValue(this) as IList;
            ...
            else if (fieldType == FieldType.Array)
            {
                ...
                var array = fd.GetValue(this) as Array;
    ...
    static private bool CheckIfHasLuaFunction(string className, string luaFunctionName, string context)
    {
        //同时匹配ClassName:LuaFunctionName和Class.LuaFunctionName两种情况
        Regex reg = new Regex(string.Format(@"function\s+{0}\s*[:.]\s*{1}[\s\S]+--make_lua_panel_tag1_do_not_delete", className, luaFunctionName));
    ...
    /// </summary>
    static private void GetLuaFunctionTextPosition(string className, string luaFunctionName, string context, out int startIndex, out int length)
    {
        startIndex = -1;
        length = 0;
