...
public class UnityEngine_UI_ImageWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("alphaHitTestMinimumThreshold", get_alphaHitTestMinimumThreshold, set_alphaHitTestMinimumThreshold),
			new LuaField("defaultETC1GraphicMaterial", get_defaultETC1GraphicMaterial, null),
	...
	static int get_alphaHitTestMinimumThreshold(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name alphaHitTestMinimumThreshold");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index alphaHitTestMinimumThreshold on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.alphaHitTestMinimumThreshold);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UnityEngine.UI.Image.defaultETC1GraphicMaterial);
		return 1;
	}
	...
	static int set_alphaHitTestMinimumThreshold(IntPtr L)
	{
		...
		obj.alphaHitTestMinimumThreshold = (float)LuaScriptMgr.GetNumber(L, 3);
		return 0;
	}
